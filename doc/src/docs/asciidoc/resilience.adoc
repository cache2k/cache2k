...
=== Don't work with expired data too long
...
Expire entries after 10 minutes. If an exceptions happens we do not want
 ...
 it is better to propagate exceptions than serving a potentially outdated value. On the other side,
 there may be temporary outages of the network for a maximum of 30 seconds we like to cover for.
