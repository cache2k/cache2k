...
To create a JCache with an additional cache2k configuration the interface `ExtendedConfiguration` and the class `MutableExtendedConfiguration`
is provided.
...
the defaults of cache2k apply. The JCache behavior can be enabled again, if needed.
====
...
Loader and Writer:: Settings in JCache configuration take precedence. If a loader or a write is specified in the
