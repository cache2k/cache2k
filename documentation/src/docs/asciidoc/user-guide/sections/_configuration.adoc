...
The configuration for the default cache manager (`CacheManager.getInstance()`) is searched via the
class loader `/cache2k.xml`. If a different class loader is provided, that is used to look for a configuration.
...
The scope prefix can also reference a parent element name. If the scope prefix is empty an value of the same level
is references.
