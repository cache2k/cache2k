...
implement more efficient loaders. When using the bulk loader and refresh ahead a
`CoalescingBulkLoader` is can be used to combine single refresh ahead requests into one bulk
request.
